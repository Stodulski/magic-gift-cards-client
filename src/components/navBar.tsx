...
export const NavBar = () => {
  ...
  const handleLogout = async () => {
    try {
      const result = await serverFetch.delete('/auth')
  ...
  return (
    <nav className='w-full py-5 px-5 flex absolute justify-between items-center'>
      ...
      <button
        ...
        className='relative cursor-pointer py-2 px-4 text-center font-barlow inline-flex justify-center text-base uppercase text-black rounded-lg border-solid transition-transform duration-300 ease-in-out group outline-offset-4 focus:outline  focus:outline-black focus:outline-offset-4 overflow-hidden'
      >
        <span className='relative z-20'>Salir</span>
        ...
        <span className='w-1/2 drop-shadow-3xl transition-all duration-300 block border-[#000] absolute h-[60%] group-hover:h-[90%] rounded-bl-lg border-l-2 border-b-2 left-0 bottom-0' />
        <span className='w-1/2 drop-shadow-3xl transition-all duration-300 block border-[#000] absolute h-[20%] rounded-br-lg border-r-2 border-b-2 right-0 bottom-0' />
      </button>
    </nav>
  )
}
